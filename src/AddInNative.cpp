...
{
	// Компонента работает ТОЛЬКО в неизолированном режиме (в адресном пространстве
	// клиента 1С): системный буфер обмена, GDI+ и оконный мониторинг требуют
	// оконного окружения клиента и в изолированном процессе не функционируют —
	// объект компоненты там не создаётся.
	//
	// НЕ МЕНЯТЬ на eCanAttachAny: заявив поддержку изоляции, компонента разрешает
	// платформе 8.3.21+ грузить её изолированно ПО УМОЛЧАНИЮ (когда тип подключения
	// не указан), и тогда на тонком клиенте x64 объект перестаёт создаваться
	// (регрессия 1.0.4.1). eCanAttachAny лишь убирал ошибку загрузки на толстом
	// клиенте x64, но рабочего объекта в изолированном процессе всё равно не давал.
